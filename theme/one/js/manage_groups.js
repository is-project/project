$(document).ready(function() {
  ...
  if(settings['form-errors'] != undefined) {
    for(i in settings['form-errors']) {
      $('form#add-edit-group-form #' + i).addClass('error');
    }
  }

  if(settings['form-status'] != undefined) {
    if(settings['form-status'] == 'edit') {
      $('form#add-edit-group-form input#project').attr('readonly', true);
      $('form#add-edit-group-form input#group').attr('readonly', true);
      $('form#add-edit-group-form input#group-name').attr('readonly', false);
      $('form#add-edit-group-form textarea#group-description').attr('readonly', false);
      $('form#add-edit-group-form input[type=submit]').attr('disabled', false);
  ...
  if(settings['form-values'] != undefined) {
    for(i in settings['form-values']) {
      $('form#add-edit-group-form #' + i).val(settings['form-values'][i]);
    }
  }

  $('table.groups td.group-settings a.details.access-1').click(function() {
    $('form#add-edit-group-form input#project').attr('readonly', true);
    $('form#add-edit-group-form input#group').attr('readonly', true);
    $('form#add-edit-group-form input#group-name').attr('readonly', true);
    $('form#add-edit-group-form textarea#group-description').attr('readonly', true);
    $('form#add-edit-group-form input[type=submit]').attr('disabled', true);
